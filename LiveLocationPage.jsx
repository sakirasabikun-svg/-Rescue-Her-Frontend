...
function LiveLocationPage() {
  ...
  const fetchCurrentLocation = () => {
    fetch(`${backendUrl}/api/location`)
      ...
      .then((data) => {
        if (data && data.area) {
          // শুধু তখনই স্টেট আপডেট হবে যখন ডাটা আসলেই চেঞ্জ হবে (অপ্রয়োজনীয় রিল্যান্ডারিং এড়াতে)
          setLocationData(prev => {
            if (prev.latitude !== data.latitude || prev.longitude !== data.longitude || prev.area !== data.area) {
              return {
                area: data.area,
  ...
  // 📍 নতুন ফিচার: প্রতি ৩ সেকেন্ড পর পর ডাটাবেজ থেকে অটো-ফেচ করা (Short Polling)
  useEffect(() => {
    // পেজ লোড হওয়ার সাথে সাথে প্রথমবার কল হবে
    fetchCurrentLocation();

    // প্রতি ৩০০০ মিলিমেকেন্ড (৩ সেকেন্ড) পর পর ব্যাকঅ্যান্ডে রিকোয়েস্ট পাঠাবে
    const intervalId = setInterval(() => {
      fetchCurrentLocation();
    }, 3000);
  ...
  const handleLocationUpdate = async (lat, lng) => {
    ...
    try {
      ...
      try {
        const backendRes = await fetch(`${backendUrl}/api/location/update`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ latitude: lat, longitude: lng, area: fullReadableArea })
        });
